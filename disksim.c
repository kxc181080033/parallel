...
void disksim_cleanup_and_printstats ()
{
  ...
  fprintf(outputfile, "cache_cmt_hit is %d, ",cache_cmt_hit);
  fprintf(outputfile, "cache_scmt_hit is %d, ",cache_scmt_hit);
  fprintf(outputfile, "cache_slcmt_hit is %d, ",cache_slcmt_hit);
  fprintf(outputfile, "flash_hit is %d\n",flash_hit);
  fprintf(outputfile, "The cache cmt hit ratio is %f\n",(cache_cmt_hit*1.0)/(rqst_cnt));
  fprintf(outputfile, "The cache scmt hit ratio is %f\n",(cache_scmt_hit*1.0)/(rqst_cnt));
  fprintf(outputfile, "The cache slmt hit ratio is %f\n",(cache_slcmt_hit*1.0)/(rqst_cnt));
  fprintf(outputfile, "The cache hit ratio is %f\n",((cache_cmt_hit+cache_scmt_hit+cache_slcmt_hit)*1.0)/(rqst_cnt));
  fprintf(outputfile, "The average valid page num of pre block erase %f\n",(all_merge_num*1.0)/(stat_erase_num));
  fprintf(outputfile, "translation page read num is %d\n",translation_read_num);
  fprintf(outputfile, "translation page write num is %d\n",translation_write_num);
  fflush(outputfile);
  disksim_cleanstats();
  disksim_printstats();

...
_u32 pm_gc_cost_benefit()
{
    ...
    int blk_cb;
    _u32 max_blk = -1, i;
    //nand_blk_num NAND中的块数
    //nand_blk 是一个结构体（fpc:free page counter; ipc:invalide page counter; lwn:last writeen page）
...
size_t pm_read(sect_t lsn, sect_t size, int mapdir_flag)
{
    ...
    sect_t lsns[SECT_NUM_PER_PAGE];  //用来储存当前LPN的4个逻辑扇区号
    ...
    //mapdir_flag == 2 是映射.. ,获取对应的映射页或是数据页的物理扇区号 
    if(mapdir_flag == 2){
...
// 暂时不知道是干什么的  估计small用来区分映射与数据
int pm_gc_get_free_blk(int small, int mapdir_flag)
{
  if (free_page_no[small] >= SECT_NUM_PER_BLK) {  // ymb？ free_gapge_no是对应free_blk_no的起始空闲扇区号，
    free_blk_no[small] = nand_get_free_blk(1);    // 即如果起始扇区号大于一个块所能有的扇区数，则要换一个块
...
// 将受害块每个页一次判断，是否有有效数据，将有效数据写到其他快上  
// 返回的benefit好像是垃圾回收的效益，回收一个块，去找一个空闲块去写入有效数据，只找一个，benefit＝0，要找多个块，benefit为负数 ymb
// small 就是表示映射区域或是数据区域
int pm_gc_run(int small, int mapdir_flag)
{
    ...
    s = k = OFF_F_SECT(free_page_no[small]);  // 模上11，取出扇区偏移量

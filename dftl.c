...
size_t opm_read(sect_t lsn, sect_t size, int mapdir_flag)
{
    ...
    if(mapdir_flag == 2){
        ...
        s_psn = opagemap[lpn].ppn * SECT_NUM_PER_PAGE;
    }
    else{
        //s_psn = hot_opagemap[lpn].ppn * SECT_NUM_PER_PAGE;   //用hot_opagemap的话，存在bug。当冷写后，如果再次读命中，hot没有改变，就会出现问题
        s_psn = opagemap[lpn].ppn * SECT_NUM_PER_PAGE;
    }
...
int opm_gc_run(int small, int mapdir_flag)
{
    ...
    s = k = OFF_F_SECT(free_page_no[small]);       //free_page_no 是空闲扇区起始位，OFF_F_SECT：获取对应页的扇区偏移
    ...
    small = -1;
    // ymb 很奇怪，page_status：1-map，0-data；而small：0-map，1-data； mapdir_flag：2-map，1-data 搞鸡毛？（找空把标志位统一一下）
    for( q = 0; q < PAGE_NUM_PER_BLK; q++){
        if(nand_blk[victim_blk_no].page_status[q] == 1){ //map block
            for( q = 0; q  < 64; q++) {
                if(nand_blk[victim_blk_no].page_status[q] == 0 || nand_blk[victim_blk_no].page_status[q] == 2){
                printf("something corrupted1=%d",victim_blk_no);
                }
            }
            small = 0;                                  // small=0 -> map block
            break;
        } 
        else if(nand_blk[victim_blk_no].page_status[q] == 0){ //data block
            for( q = 0; q  < 64; q++) {
                if(nand_blk[victim_blk_no].page_status[q] == 1 || nand_blk[victim_blk_no].page_status[q] == 2){
                    printf("something corrupted2",victim_blk_no);
                }
            }
            small = 1;                                  // small＝1 -> data block
            break;
        }
        else if(nand_blk[victim_blk_no].page_status[q] == 2){
            for( q = 0; q  < 64; q++) {
                if(nand_blk[victim_blk_no].page_status[q] == 1 || nand_blk[victim_blk_no].page_status[q] == 0){
                    printf("something corrupted3",victim_blk_no);
                }
            }
            small = 2;                                  // small＝2 ->hot data block
            break;
        }
    }
    ...
    pos = 0;
    merge_count = 0;
    for (i = 0; i < PAGE_NUM_PER_BLK; i++)   // 这个for语句就是将受害块中有效的页重新写到新的分配的块中（快写应该这里有所操作）
    {
        ...
        if(valid_flag == 1)
        {
            ...
            {                       
                ...
                if(nand_blk[victim_blk_no].page_status[i] == 0){
                    ...
                    opagemap[BLK_PAGE_NO_SECT(copy_lsn[s])].ppn = BLK_PAGE_NO_SECT(SECTOR(free_blk_no[small], free_page_no[small]));
                    hot_opagemap[BLK_PAGE_NO_SECT(copy_lsn[s])].ppn = BLK_PAGE_NO_SECT(SECTOR(free_blk_no[small], free_page_no[small]));
                    nand_page_write(SECTOR(free_blk_no[small],free_page_no[small]) & (~OFF_MASK_SECT), copy_lsn, 1, 0);
                    free_page_no[small] += SECT_NUM_PER_PAGE;
                }
                
                if((opagemap[BLK_PAGE_NO_SECT(copy_lsn[s])].map_status == MAP_REAL) || (opagemap[BLK_PAGE_NO_SECT(copy_lsn[s])].map_status == MAP_GHOST)) {   //判断是否在缓存中
                    delay_flash_update++;
                }
            
                else {                          // 如果不在缓存中，需要从NAND flash中读取
    
                    map_arr[pos] = copy_lsn[s];  //暂存未命中的逻辑山区号 pos表示未命中个数
                    pos++;
                } 
            }
        }
    }
    // 这下面貌似就是一些变量的统计，有一丝复杂
    ...
    for(i =0 ; i < pos; i++) {
        old_flag = 0;
        for( j = 0 ; j < k; j++) {
            if(temp_arr[j] == mapdir[((map_arr[i]/SECT_NUM_PER_PAGE)/MAP_ENTRIES_PER_PAGE)].ppn) {   //判断是否在同一翻译页上
                if(temp_arr[j] == -1){
...
size_t opm_write(sect_t lsn, sect_t size, int mapdir_flag)  
{
    ...
    if (free_page_no[small] >= SECT_NUM_PER_BLK) 
    {
        // 如果空闲块获取成功，free_page_no重置，否则执行垃圾回收后，重新获取空闲块
    ...
    else{  //mapdir_flag ==0   hot data
        hot_opagemap[lpn].ppn = ppn;
        opagemap[lpn].ppn = ppn;
    }
...
int opm_init(blk_t blk_num, blk_t extra_num)
{
    ...
    hot_opagemap = (struct omap_dir *)malloc(sizeof(struct omap_dir) * opagemap_num);   // ymb..
    mapdir = (struct omap_dir *)malloc(sizeof(struct omap_dir) * opagemap_num / MAP_ENTRIES_PER_PAGE); 

    if ((opagemap == NULL) || (mapdir == NULL) || (hot_opagemap == NULL)) {
        return -1;
    }
    ...
    memset(hot_opagemap,  0xFF, sizeof (struct omap_dir) * hot_num);
    ...
    free_blk_no[0] = nand_get_free_blk(0);   //0-map
    free_page_no[0] = 0;
    free_blk_no[1] = nand_get_free_blk(0);   //1-data
    free_page_no[1] = 0;
    free_blk_no[2] = nand_get_free_blk(0);   //2-hot
    free_page_no[2] = 0;
